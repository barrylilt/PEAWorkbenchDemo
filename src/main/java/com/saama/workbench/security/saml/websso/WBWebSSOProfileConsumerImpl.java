 ...
 */
package com.saama.workbench.security.saml.websso;
 ...
 */
public class WBWebSSOProfileConsumerImpl extends AbstractProfileBase implements WebSSOProfileConsumer {
	
	private static Logger log = Logger.getLogger(WBWebSSOProfileConsumerImpl.class);

    public WBWebSSOProfileConsumerImpl() {
    }

    public WBWebSSOProfileConsumerImpl(SAMLProcessor processor, MetadataManager manager) {
    ...
    protected void verifyAudience(SAMLMessageContext context, List<AudienceRestriction> audienceRestrictions) throws SAMLException {
        ...
        for (AudienceRestriction rest : audienceRestrictions) {
        	log.debug("DEFAULT_ELEMENT_NAME - " + rest.DEFAULT_ELEMENT_NAME);
        	log.debug("TYPE_NAME - " + rest.TYPE_NAME);
            ...
            for (Audience aud : rest.getAudiences()) {
            	...
            	log.debug("context.getLocalEntityId() - " + context.getLocalEntityId());
            	log.debug("aud.getAudienceURI() - " + aud.getAudienceURI());
//            	if (aud.getAudienceURI().equalsIgnoreCase("https://peajavasfa-ws05.upeadev.com/PEAWorkbench/saml/metadata") || context.getLocalEntityId().equals(aud.getAudienceURI())) {
//            		continue audience;
//            	}
